...
class GateContext:
    ...
    bot_nicknames: str = ""
    # bot 的昵称列表，注入 Gate prompt 帮助 LLM 识别变体称呼 (如 "小暮、暮暮、洛宝")
...
- 有人在讨论你但不对你说话（如"小暮说的参数对吗"是对群友说的）
- 有人在对{peer_bot_name}说话时提到你（如"你觉得小暮怎么样"→ 说话对象是{peer_bot_name}）
- 有人在自言自语/发纯表情/灌水
...
- ★ 鼓励发图！send_sticker 是常驻工具无需推荐，只需在 suggested_sticker_mood 填入表情标签:
  开心/得意/兴奋 → 开心/得意 | 害羞/被夸/被戳穿 → 害羞 | 生气/吃醋/被惹到 → 生气
  难过/委屈/心疼 → 难过 | 惊讶/慌张/被吓到 → 惊讶 | 撒娇/卖萌/贴贴 → 卖萌/点赞
  想逗人/撩一下 → 挑逗/打趣 | 围观/吃瓜/八卦 → 吃瓜 | 吐槽/嫌弃 → 无语
  敷衍/摆烂/不想说话 → 无语 | 安慰人/深夜聊天 → 点赞/卖萌 | 打招呼/冒泡 → 问好
...
- summary: 用1-2句话描述群聊当前在聊什么、气氛如何。
  要具体，不要泛泛而谈。例如"大家在聊时间问题，搞错了时间被多人纠正"而不是"群友在聊天"。
...
★ 昂贵工具新规: 即使用户可使用, 也只在明确要求时才推荐。
  ...
  裸图无文字 → 不加 describe_image — 表情包不需要 AI 分析。
...
★ 昂贵工具下发新规:
  1. search_knowledge/web_search: 仅限明确「搜/查/搜索」关键词 + 搜的是文字信息(资料/教程/文档/新闻)
...
- reply_target: {{"user_id": "...", "user_name": "..."}} — 最该回复的用户
  ★ 传话/代话场景: reply_target 仍是说话者本人（触发人）。
...
- target_bot: "" 或 "moon"
- reason: 一句话说明为什么
- suggested_action: 具体动作 (如 "@ 别纠结时间了")
...
★ 语义分析三步法 — 每条消息都按这个顺序判断:
  ...
  Step 2: 找动词主语 — "你"指谁？
    ...
    例: "@{bot_name} ，你觉得她说的对吗？" → 呼语是{peer_bot_name}()，"你"={peer_bot_name}
...
{{"group_context": {{"atmosphere": "water_chat/tech_discussion/teasing_bot/banter/serious_help/chaotic/argumentative",
 ...
 "model_tier": "lite/pro",
 "reasoning_effort": "low/medium/high/max",
 "cross_bot_action": null | {{"should_intervene": true, "target_bot": "/moon", "reason": "...", "suggested_action": "..."}},
 "should_profile": true/false,
 "reply_target": {{"user_id": "...", "user_name": "..."}},
 "reasoning": "..."}}"""
...
class IntentGate:
    ...
    async def evaluate_intent(
        ...
        # 工具摘要
        tools_summary = "、".join(ctx.available_tools[:8]) if ctx.available_tools else "无"
        if len(ctx.available_tools) > 8:
            tools_summary += f" 等{len(ctx.available_tools)}个"
    ...
    async def evaluate_full(
        ...
        usable_tools_summary = "、".join(usable_list[:8]) if usable_list else "无"
        if len(usable_list) > 8:
            usable_tools_summary += f" 等{len(usable_list)}个"
        ...
        try:
            ...
            return FullGateResult(
                directed_to_me=False,
                should_reply=False,
                relevance_reasoning="FullGate 超时, fail-closed",
                parse_ok=False,
                parse_error="超时",
            ...
            return FullGateResult(
                directed_to_me=False,
                should_reply=False,
                relevance_reasoning="FullGate 异常, fail-closed",
                parse_ok=False,
                parse_error="异常",
                fast_path=_fast_path,
            )
        ...
        if not raw:
            return FullGateResult(
                directed_to_me=False,
                should_reply=False,
                relevance_reasoning="FullGate 空响应, fail-closed",
                parse_ok=False,
                parse_error="空响应",
                fast_path=_fast_path,
            )
...
if __name__ == "__main__":
    ...
    ctx = GateContext(
        bot_name="暮恩",
        bot_identity="绿毛蛇女 AI 助手",
        peer_bot_name="",
        peer_bot_qq="BOT_QQ_ALT",
        trigger_uid="123456",
        is_at_mention=True,
    )
