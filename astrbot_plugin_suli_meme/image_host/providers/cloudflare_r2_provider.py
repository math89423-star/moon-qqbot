...
try:
    ...
    raise ImportError(
        "Cloudflare R2 图床需要 boto3/botocore，请手动安装: pip install boto3 botocore"
    ) from e
...
class CloudflareR2Provider(ImageHostInterface):
    ...
    def upload_image(self, file_path: Path) -> ImageInfo:
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.error(f"上传异常: {e!s}")
                if attempt < max_retries - 1:
                    time.sleep(retry_delay)
                    continue
                raise CloudflareR2Error(f"上传失败: {e!s}")
    ...
    def _parse_s3_key(self, s3_key: str) -> tuple:
        """解析S3键名获取分类和文件名"""
        # 移除 memes/ 前缀
        s3_key = s3_key.removeprefix("memes/")
