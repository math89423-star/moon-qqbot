    ...
    def upload_image(self, file_path: Path) -> ImageInfo:
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.error(f"网络错误: {e!s}，重试中...")
                time.sleep(retry_delay)
                continue
            except Exception as e:
                logger.error(f"上传异常: {e!s}，重试中...")
                time.sleep(retry_delay)
                continue
                    ...
                    if response.status_code == 200:
                        ...
                        if "invalid timestamp" in result.get("message", "").lower():
                            if attempt < max_retries - 1:
                                logger.warning("时间戳错误，重试中...")
                                time.sleep(retry_delay)
                                continue
                        if "invalid nonce" in result.get("message", "").lower():
                            if attempt < max_retries - 1:
                                logger.warning("nonce错误，重试中...")
                                time.sleep(retry_delay)
                                continue
                        # 其他错误，记录日志但继续尝试下一页
                        logger.warning(
