...
from .providers import StarDotsProvider, _load_cloudflare_r2_provider
...
class ImageSync:
    ...
    def __init__(
        ...
        if provider_type == "stardots":
            ...
            _R2Provider = _load_cloudflare_r2_provider()
            self.provider = _R2Provider(config)
        else:
    ...
    async def start_sync(self, task: str) -> bool:
        ...
        if task == "upload" and not status.get("to_upload"):
            ...
            return True
        if task == "download" and not status.get("to_download"):
            logger.info("没有文件需要下载")
            return True
        if task == "overwrite_to_remote" and not (
            status.get("to_upload") or status.get("to_delete_remote")
        ):
            logger.info("云端已是最新且完全一致，无需覆盖")
            return True
        if task == "overwrite_from_remote" and not (
...
def run_sync_process(config: dict[str, str], local_dir: str, task: str):
    ...
    try:
        ...
        logger.exception(f"同步进程发生异常: {e!s}")
        sys.exit(1)
