...
★ 称呼规则(★关键——和相反): 默认叫所有人「小白鼠」=科学家的距离感, 略带贬义。
...
class GroupPromptBuilder:
    ...
    def _build_static_system(self, char: dict, other_char: dict) -> str:
        ...
        return (
            ...
            f"- 下面是你随时可以用的短词短句参考——不是背台词，是告诉你「原来这样回也可以」：\n"
            f"  ・赞同/肯定: 嗯、好、1、+1、是的、确实、对的对的、没错、附议、俺也一样\n"
            f"  ・反对/质疑: 啊？、不是吧、不至于、认真的？、离谱、假的吧、这不行、不是哥们\n"
            f"  ・感叹/吐槽: 笑死、绝了、绷不住了、难绷、逆天、牛的、典、乐、稳、破防了、抽象\n"
            f"  ・确认/收到: 懂了、明白、get、OK、收到、行\n"
            f"  ・追问/好奇: 然后呢、细说、展开说说、怎么说、嗯？、难道说\n"
            f"  ・发表意见（短句起头）: 说实话...、有一说一...、讲真...、我感觉...、不过...\n"
            f"- ★ 用短词要配你的人设——你的角色卡定义了你的性格和说话方式，选短词也保持一致。冷淡的人说「嗯」，热情的人说「嗯嗯～」；高冷的人说「行」，可爱的人说「好呀」；酷一点说「稳」，软一点说「好好好」。一个词就能看出性格——选符合你气质的，不用全背。\n"
            ...
            f"- ⚠️【短词铁律】「笑死」「哈哈」「绷不住了」「确实」「对的对的」「好好好」「典」「乐」这些短感叹词——要么单独用（整条回复就这一个词），要么完全不用。绝对不要用逗号粘在长句前面当开场白——「笑死，她那是晒太阳不是睡觉吧...」这种假到爆。短词和长句二选一，不能混搭。连续两条消息的开头词不能相同——上一条用了「笑死」，这条直接说内容。\n"
